...
class NoteTest < ActiveSupport::TestCase
  ...
  def test_display_name
    Note.all.each do |note|
      expected = note.letter.upcase
      expected = "#{expected} Flat"  if note.flat
      expected = "#{expected} Sharp" if note.sharp
      assert_equal expected, note.display_name, "Expected '#{expected}', but got #{note.display_name}."
    end
  end
end
